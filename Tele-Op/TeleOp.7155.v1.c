...
#pragma config(Motor,  motorC,          winch,         tmotorNXT, PIDControl, encoder)
...
#pragma config(Servo,  srvo_S1_C1_6,    sideHook,             tServoContinuousRotation)
...
task hook2 () {
	...
	while(true) {
		...
		//Put hook up
		if(joy1Btn(Btn3)&&!hookUp) { 	//If the A Button is pressed, and the hook isn't up
			servo[sideHook] = 113;				//Set right hook to up position
			hookUp=true;								//Tell the program that the hook is up
			wait1Msec(300);							//Wait so that we can give the hook time to go up, and so the button doesn't get "stuck"
		}

		//Put hook down
		if(joy1Btn(Btn3)&&hookUp) {
			servo[sideHook] = 100;
			hookUp=false;
			//while(joy1Btn(Btn2)) {}
			wait1Msec(300);
